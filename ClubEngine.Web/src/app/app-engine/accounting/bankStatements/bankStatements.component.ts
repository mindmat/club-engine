...
@Component({
  ...
  imports: [MatFormFieldModule, FileUploadComponent, FuseCardComponent, MatIconModule, TranslateModule, DatePipe, CommonModule, ReactiveFormsModule]
})
export class BankStatementsComponent implements OnInit
{
  ...
  ngOnInit(): void
  {
    this.eventService.selected$.subscribe(event => this.uploadUrl = `api/${event.acronym}/upload/SavePaymentFileCommand`);
