...
export class NavigationService {
   ...
   * Constructor
   */
  constructor(eventService: PartitionService, translateService: TranslateService, menuService: MenuService) {
    combineLatest([
      translateService.onLangChange.asObservable().pipe(
        map((e) => e.lang),
  ...
  getBadgeStyle(content: MenuNodeContent): string | null {
    ...
    switch (content.style) {
      case MenuNodeStyle.Info:
        return 'px-2 bg-sky-600 text-black rounded-full';
      case MenuNodeStyle.ToDo:
        return 'px-2 bg-yellow-500 text-black rounded-full';
      case MenuNodeStyle.Important:
        return 'px-2 bg-red-500 text-black rounded-full';
      default:
        return 'px-2 bg-sky-600 text-black rounded-full';
    }
  }
