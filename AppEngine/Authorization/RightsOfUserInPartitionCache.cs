...
public class RightsOfUserInPartitionCache(IQueryable<UserInPartition> usersInPartitions,
                                          ...
                                          IEnumerable<IRightsOfPartitionRoleProvider> rightsOfPartitionRoleProviders)
{
    ...
    private async Task<HashSet<string>> CreateRightsOfUserInPartitionCacheEntry(ICacheEntry entry, Guid userId, Guid partitionId)
    {
        ...
        return rightsOfPartitionRoleProviders.SelectMany(rpr => rpr.GetRightsOfPartitionRoles(partitionId, usersRolesInPartition))
                                             .OrderBy(rgt => rgt)
                                             .ToHashSet();
    }
}
